...
export async function getCustomSectionItemByUsers(user_id) {
  const result = await query(`SELECT * FROM custom_section_item WHERE user_id = $1;`, [
    user_id,
  ]);
  return result.rows;
}
...
export async function updateCustomSectionItem(body) {
 ...
 const text = body.text;
 const due_date = body.due_date;
 const section_id = body.customise_id;
  const data = await query(
    `UPDATE custom_section_item SET user_id = $1, text=$2, due_date=$3 WHERE section_id = $7 RETURNING appointment;`,
    [user_id,
        section_id,
        text,
        due_date ]
